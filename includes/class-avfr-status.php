...
class Avfr_Status {
	...
	*/
	function avfr_change_status() {
		...
		if ( 'on' == avfr_get_option('avfr_send_mail_'.$new_status.'_writer','avfr_settings_mail') ) {
			...
			wp_mail( $reciever_email, __( 'Feature Request ', 'feature-request' ).$entry->post_title.' '.$new_status.'.', $mail_content );
		...
		if ( 'on' == avfr_get_option('avfr_send_mail_'.$new_status.'_voters','avfr_settings_mail') ) {
			...
			$mail_content    = str_replace($search, $replace, $content);
			wp_mail( $reciever_emails, __( 'Request ', 'feature-request' ).$entry->post_title.' '.$new_status.'.', $mail_content );
	...
	*/
	function avfr_mail_status( $status, $postid ) {
		...
		$message = __( "The status of ", 'feature-request' ).$entry->post_title.__( " has been updated to:\n", 'feature-request' );
		$message .= "".$status."\n\n";
		$message .= __( "Manage features at link below\n", '' );
		$message .= "".wp_login_url()."\n\n";

		if ( !$mail_disabled )
			wp_mail( $admin_email, __( 'Feature Request ', 'feature-request' ).$postid.__( ' Approved ', 'feature-request' ), $message );
